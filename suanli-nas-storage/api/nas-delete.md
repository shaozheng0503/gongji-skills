...
> 远端最后更新：2026-09-11T08:52:42.000Z
> 端点：`POST /api/storage/nas/v1/delete`
...
删除集群存储卷。Active 卷受理成功后 status 为 Deleting。Exception 卷删除后 status 为 Deleted。存在进行中的互传任务时不可删除，除非 force 为 true。
...
- `id` `integer` **(必填)**：卷 ID
- `force` `boolean`：是否强制删除 — 为 true 时跳过进行中互传任务等前置校验。默认 false
- `reason` `string | null`：删除原因
