...
> 远端最后更新：2026-09-11T08:50:34.000Z
> 端点：`POST /api/storage/nas/v1/encrypt/s3/retry`
...
对 Exception 或 Error 状态的任务重新提交。原任务将标记为 Deleted，并创建新任务。应答中的 id 为新任务 ID，status 为 Queuing。
...
- `id` `integer` **(必填)**：原任务 ID — 原任务状态必须为 Exception 或 Error
- `storage_id` `integer` **(必填)**：卷 ID — 可与原任务不同，必须为 Active
...
- `s3_prefix` `string | null`：对象前缀
- `nas_path` `string | null`：集群存储路径
- `overwrite_policy` `any`
- `ignore_patterns` `array | null`：忽略规则
