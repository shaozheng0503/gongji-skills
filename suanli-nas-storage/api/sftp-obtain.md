...
> 来源：Apifox 官方文档（实时同步） · 分组：共绩算力 Open API/存储/集群存储/SFTP
> 远端最后更新：2026-09-11T08:59:39.000Z
> 端点：`POST /api/storage/nas/v1/sftp/obtain`
...
- `storage_id` `integer` **(必填)**：卷 ID — 必须大于 0
...
- `data` `object` **(必填)**
  - `id` `integer` **(必填)**：会话 ID
  - `status` `any` **(必填)**
  - `username` `string | null`：用户名 — 仅 status 为 Running 时返回
  - `password` `string | null`：密码 — 仅 status 为 Running 时返回
  - `sftp_url` `string | null`：SFTP 地址 — 格式为 host:port。仅 status 为 Running 时返回
  - `web_url` `string | null`：Web 入口 — 仅 status 为 Running 时返回
  - `webdav_url` `string | null`：WebDAV 入口 — 仅 status 为 Running 时返回
