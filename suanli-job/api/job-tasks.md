...
> 远端最后更新：2026-07-03T03:56:00.000Z
> 端点：`GET /api/task/job/search`
...
| `group_task` | boolean | 否 | "true"/"false"，备注：注意区分大小写 |
| `group_id` | integer | 否 | — |
...
- `data` `object | null` **(必填)**
  ...
  - `results` `array` **(必填)**
    - **array<object>**
      - `task_id` `integer` **(必填)**
      - `task_name` `string` **(必填)**
      - `task_type` `string` **(必填)**，枚举: `Job`
      - `sub_type` `string | null`，枚举: `Spot`
      - `resources` `array` **(必填)**
        - **array<object>**
      ...
      - `schedule_status` `string | null`，枚举: `Scheduled`/`Queued`
      - `points` `integer` **(必填)**
