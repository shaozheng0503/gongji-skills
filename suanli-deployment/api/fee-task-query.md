...
> 远端最后更新：2026-06-24T07:23:18.000Z
> 端点：`GET /api/billing/get_task_billing_record`
...
| `start_time` | string | 是 | 开始时间(RFC3339格式)（示例：2026-01-01T00:00:00+08:00） |
| `end_time` | string | 是 | 结束时间(RFC3339格式)（示例：2026-01-02T23:59:59+08:00） |
| `page` | number | 否 | 页码 |
...
- `data` `object | null` **(必填)**
  ...
  - `results` `array` **(必填)**
    - **array<object>**
      ...
      - `task_type` `string | null` **(必填)**：任务类型，枚举: `Deployment`/`Development`
      - `task_name` `string | null` **(必填)**：任务名
